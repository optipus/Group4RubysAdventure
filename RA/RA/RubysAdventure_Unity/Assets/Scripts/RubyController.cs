        ...
        if (Input.GetKeyDown(KeyCode.X))
{
    ...
    if (hit.collider != null)
    {
        ...
        //Aded similar TextBox Popup mechanic to that of Jambi -- Luke B.
    hit = Physics2D.Raycast(rigidbody2d.position + Vector2.up * 0.2f, lookDirection, 1.5f, LayerMask.GetMask("NPC"));
    if (hit.collider != null)
    {
        NonPlayerBear bearCharacter = hit.collider.GetComponent<NonPlayerBear>();
        if (bearCharacter != null)
        {
            bearCharacter.DisplayDialog();
        }
    }
        //Aded similar TextBox Popup mechanic to that of Jambi -- Luke B.
    hit = Physics2D.Raycast(rigidbody2d.position + Vector2.up * 0.2f, lookDirection, 1.5f, LayerMask.GetMask("NPC"));
    if (hit.collider != null)
    {
        NonPlayerBandit banditCharacter = hit.collider.GetComponent<NonPlayerBandit>();
        if (banditCharacter != null)
        {
            banditCharacter.DisplayDialog();
        }
    }
        //Aded similar TextBox Popup mechanic to that of Jambi -- Luke B.
    hit = Physics2D.Raycast(rigidbody2d.position + Vector2.up * 0.2f, lookDirection, 1.5f, LayerMask.GetMask("NPC"));
    if (hit.collider != null)
    {
        PlantyScript banditCharacter = hit.collider.GetComponent<PlantyScript>();
        if (banditCharacter != null)
        {
            banditCharacter.DisplayDialog();
        }
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        ...
        //Slowdown Slime Traps -- Luke B
         if (other.gameObject.CompareTag("SpeedSlow"))
        {
            speed = speed - 2;
            PlaySound(Slow);
